...
#   但问句「确定要…？」里的「确定」= sure，不同义 → 需按场景裁决，勿整体合并。
# 逾期 = 超期：同 overdue —— 状态「已超期」、配置「订单超期/超期天数」、规则「逾期规则/逾期归还」。
# 取用 = 领取, 取车：pickup —— 时间线「待取用/取用时间」vs 领取规则「领取车辆/电池」；语义微差。
# 远程锁定 = 远程锁车, 锁车：当前未形成分裂，暂缓。
